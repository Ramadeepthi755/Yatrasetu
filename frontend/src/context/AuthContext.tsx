...
export function AuthProvider({ children }: { children: React.ReactNode }) {
    ...
    async function initSession() {
      try {
        if (isSupabaseConfigured) {
          ...
          // Check local stored demo session if any
          const savedUser = localStorage.getItem('yatrasetu_mock_user');
          const savedToken = localStorage.getItem('yatrasetu_mock_token');
          if (savedUser && savedToken) {
            const parsed = JSON.parse(savedUser);
            setUser(parsed);
            setToken(savedToken);
            if (parsed.role === 'PARTNER') {
              const savedPartner = localStorage.getItem('yatrasetu_mock_partner');
  ...
  const login = async (email: string, password?: string) => {
    ...
    try {
      if (isSupabaseConfigured && password) {
        ...
        setToken(mockJwt);
        const res = await syncUserSession(`mock-${email}`, email, email.split('@')[0], 'TRAVELER', undefined, mockJwt);
        setUser(res.data);
        localStorage.setItem('yatrasetu_mock_user', JSON.stringify(res.data));
        localStorage.setItem('yatrasetu_mock_token', mockJwt);
      }
    } finally {
      setLoading(false);
    }
  ...
  const signup = async (
    ...
    try {
      if (isSupabaseConfigured && password) {
        ...
        const mockJwt = `mock-${role.toLowerCase()}-${email}`;
        setToken(mockJwt);
        const res = await syncUserSession(`mock-${email}`, email, name, role, partnerSubtype, mockJwt);
        setUser(res.data);
        localStorage.setItem('yatrasetu_mock_user', JSON.stringify(res.data));
        localStorage.setItem('yatrasetu_mock_token', mockJwt);
        if (role === 'PARTNER') {
          ...
          localStorage.setItem('yatrasetu_mock_partner', JSON.stringify(pData));
        }
      }
    } finally {
      setLoading(false);
    ...
    try {
      ...
      try {
        ...
        if (demoRole === 'GOVERNMENT') {
          profileData.role = 'GOVERNMENT'; // For mock dev state
        }
      } catch (e) {
      ...
      setUser(profileData);
      setToken(mockJwt);
      localStorage.setItem('yatrasetu_mock_user', JSON.stringify(profileData));
      localStorage.setItem('yatrasetu_mock_token', mockJwt);
      ...
      if (demoRole === 'PARTNER') {
        ...
        localStorage.setItem('yatrasetu_mock_partner', JSON.stringify(pDetails));
      } else {
        setPartnerDetails(null);
  ...
  const updatePartner = async (data: Partial<PartnerProfile>) => {
    ...
    localStorage.setItem('yatrasetu_mock_partner', JSON.stringify(res.data));
  };
